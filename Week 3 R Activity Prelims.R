...
womendata=data.frame(ht,wt)
names(womendata)[1] <- 'height'
names(womendata)[2] <- 'weight'
...
ggplot(womendata, aes(x=height, y=weight)) + 
  geom_point(shape = 5, colour = "orange", fill = "white", size = 2, stroke = 1)
